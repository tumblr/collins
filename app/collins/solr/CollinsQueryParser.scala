...
class CollinsQueryParser private (val docTypes: List[SolrDocType]) extends JavaTokenParsers {
  ...
  def range = ident ~ "=" ~ "\\[|\\(".r ~ rangeValueOpt ~ "," ~ rangeValueOpt ~ "\\]|\\)".r ^^ {
    case key ~ "=" ~ open ~ low ~ "," ~ high ~ close => (open, close) match {
  ...
  def compareOp = ident ~ "<=|>=|<|>".r ~ rangeValue ^^ {
    case key ~ op ~ value => op match {
