...
case class AssetMetaValue(@Column("ASSET_ID") assetId: Long,
  ...
  // TODO:
  override def compare(z: Any) = true
}
...
object AssetMetaValue extends Schema with BasicModel[AssetMetaValue] with AssetMetaValueKeys {
  ...
  def apply(asset: Asset, assetMetaId: Long, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, assetMetaId, 0, value)
  def apply(asset: Asset, asset_meta: AssetMeta.Enum, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, asset_meta.id, 0, value)
  def apply(asset: Asset, assetMetaId: Long, groupId: Int, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, assetMetaId, groupId, value)
  def apply(asset: Asset, asset_meta: AssetMeta.Enum, groupId: Int, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, asset_meta.id, groupId, value)
  def apply(asset: Asset, assetMeta: AssetMeta, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, assetMeta.id, 0, value)
  def apply(asset: Asset, assetMeta: AssetMeta, groupId: Int, value: String): AssetMetaValue =
    AssetMetaValue(asset.id, assetMeta.id, groupId, value)
      ...
      log {
        ...
        val assets = from(Asset.tableDef)(asset =>
          where(whereClause(asset) and afinder.asLogicalBoolean(asset))
            select (asset)).distinct.page(page.offset, page.size).toList
  ...
  protected def excludes(asset: Asset, toFind: AssetMetaFinder, bool: Option[String]): Option[LogicalBoolean] = {
    val clauses = toFind.filter(_._2.isEmpty)
    if (clauses.length == 0) {
      return None
    }
    // Don't need value for excludes match, just want assets that have a value
    val subqueries = clauses.map {
      case (am, v) =>
        from(tableDef)(a =>
  ...
  protected def includes(asset: Asset, toFind: AssetMetaFinder, bool: Option[String]): Option[LogicalBoolean] = {
    val operator = bool.toBinaryOperator
    val clauses = toFind.filter(_._2.nonEmpty)
    if (clauses.length == 0) {
      return None
    }
    val clauseExpressions = clauses.map {
      case (am, v) =>
